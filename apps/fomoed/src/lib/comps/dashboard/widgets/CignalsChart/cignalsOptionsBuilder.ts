import type { CignalsDatapointFetchOptions, CignalsTimestep } from '$lib/types';
...
export class CignalsOptionsBuilder {
	...
	get endRange() {
		...
		const alignedTimeGMT = endTimeGMT
			...
			.add(value - (endTimeGMT.get(alignUnit) % value), alignUnit); // Move to the next alignment point
	...
	setPriceStep(step: number | 'auto'): CignalsOptionsBuilder {
		if (step !== 'auto' && step <= 0) {
			throw new Error("price_step can't be less than or equal to 0. Got: " + step);
		}
