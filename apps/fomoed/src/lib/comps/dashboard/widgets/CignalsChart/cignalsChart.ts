...
import {
	...
	rgbToString,
	type RGB
} from '$ts/client/utils/ui';
import { timeIntervalStringToMs } from '$ts/utils/server/parse';
import {
...
export class CignalsChart {
	...
	drawCandlesticks(data: PriceDataArray) {
		...
		for (let i = 0; i < data.length; i++) {
			...
			const positionedClose =
				((close - this._shownMinPrice) / (this._shownMaxPrice - this._shownMinPrice)) * height;
			const positionedOpen =
				((open - this._shownMinPrice) / (this._shownMaxPrice - this._shownMinPrice)) * height;
			...
			const positionedHigh =
				((high - this._shownMinPrice) / (this._shownMaxPrice - this._shownMinPrice)) * height;
			const positionedLow =
				((low - this._shownMinPrice) / (this._shownMaxPrice - this._shownMinPrice)) * height;
	...
	drawFootprints() {
		...
		const footprints: (FootprintData & { candleIndexInVisibleCandles: number })[] =
			...
			if (footprint.timestamp === lastCandleTs && this._highlightFootprintPrice !== null) {
				...
				if (
					...
					const highlightRectWidth =
						this._footprintDisplayMode === 'buy-sell' ? rectWidth / 2 : rectWidth;
	...
	#getPriceTickY(price: number) {
		...
		return (
			height -
			((price - this._shownMinPrice) / (this._shownMaxPrice - this._shownMinPrice)) * height
		);
	}
	...
	drawCurrentPrice() {
		...
		const yPos =
			height -
			((currentPrice - this._shownMinPrice) / (this._shownMaxPrice - this._shownMinPrice)) * height;
	...
	#drawBottomVolumeArea() {
		...
		if (this._barAreaDisplayMode === 'delta') {
			drawRes = this._drawDeltaVolumeBars({ barAreaHeight: maxBarHeight, barAreaBottomOffset });
	...
	#drawRightSideVolumeArea() {
		type VolInfo = { buyVolume: number; sellVolume: number; delta: number; total: number };

		const prices = this._filteredFootprints.map((fp) => fp.price);
		const priceToVolInfo: { [price: number]: VolInfo } = {};

		for (const price of prices) {
			priceToVolInfo[price] = { buyVolume: 0, sellVolume: 0, delta: 0, total: 0 };
		}
	...
	private _drawSplitVolumeBars({
		...
		for (let i = 0; i < agg.length; i++) {
			...
			ctx.fillRect(
				xPos + barOffsetX,
				~~(height - (a.relativeBuyHeight + a.relativeSellHeight) * barAreaHeight) +
					1 -
					barAreaBottomOffset,
				barWidth,
	...
	private _drawDeltaVolumeBars({
		...
		for (let i = 0; i < totalVolumes.length; i++) {
			...
			const yPos =
				this.canvas.clientHeight - zeroRelativeHeight * barAreaHeight - barAreaBottomOffset;
	...
	#pointInTimeFromCanvasX(x: number): number {
		...
		const tsAtPointer =
			(rightSideTimestamp - leftSideTimestamp) * normalizedPointerPos + leftSideTimestamp;
	...
	#handleModeChange(event: PointerEvent) {
		const isInsideBarArea =
			event.clientY > this.canvas.clientHeight * (1 - this._barAreaHeightNormalized);
		...
		const isInsideChartArea =
			event.clientY < this.chartAreaHeight && event.clientX < this._chartAreaRight;
		...
		const isInRightVolumeArea =
			event.clientX > this.rightVolAreaLeft && event.clientY < this.chartAreaHeight;
		...
		for (const datapoint of this._data) {
			for (const fp of datapoint.footprints) {
				let tradedVol: TradedVol | null =
					tradedVols.find((tv) => tv.ts === datapoint.candle.timestamp && tv.price === fp.price) ||
					null;
				if (!tradedVol) {
					tradedVol = { ts: datapoint.candle.timestamp, price: fp.price, vol: 0 };
