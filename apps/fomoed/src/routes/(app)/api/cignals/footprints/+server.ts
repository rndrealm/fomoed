...
import type { CignalsFootprintFetchOptions } from '$lib/types';
import { timeIntervalStringToMs } from '$ts/utils/server/parse';
import { CIGNALS_HOST } from '$ts/server';
...
export const GET: RequestHandler = async ({ url }) => {
	...
	if (!instrument_id) return new Response('Missing instrument_id', { status: 400 });

	const start_range = parseInt(url.searchParams.get('start_range') ?? '');
	if (isNaN(start_range)) return new Response('Invalid start_range', { status: 400 });

	const end_range = parseInt(url.searchParams.get('end_range') ?? '');
	if (isNaN(end_range)) return new Response('Invalid end_range', { status: 400 });

	const price_step = parseFloat(url.searchParams.get('price_step') ?? '');
	if (isNaN(price_step)) return new Response('Invalid price_step', { status: 400 });
	...
	for (let timestamp = start_range; timestamp < end_range; timestamp += msStep) {
	...
	const fetchPromises = timestampsToFetch.map((timestamp) => {
		...
		const options: CignalsFootprintFetchOptions = {
			instrument_id,
		...
		return fetch(url.toString(), {
			method: 'GET',
			headers: {
				Authorization: PRIVATE_CIGNALS_KEY
			}
		}).then((response) => {
			if (!response.ok) {
				console.error(`Error fetching footprints from cignals: ${response.statusText}`);
				return [];
			}
