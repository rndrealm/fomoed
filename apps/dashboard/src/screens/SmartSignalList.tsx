...
import { smartSignalsAtom, useSmartSignals } from "./hooks/use-smart-signals";
...
const SmartSignalItem = ({
  ...
  <div className="group flex items-start p-4 rounded-md border border-[#333333] bg-[#222222] hover:bg-[#2A2A2A] transition-colors relative">
    ...
    <div className="flex-1 pl-3">
      ...
      <span
        className={`text-xs pt-1 block ${firedAt ? "text-green-400" : "text-yellow-400"}`}
      >
        {firedAt
          ? `Fired at: ${dayjs(firedAt).format("DD MMM YYYY, HH:mm")}`
          : "Not fired yet"}
      </span>
    </div>
    {onDelete && (
...
export default function SmartSignalList() {
  ...
  console.log("🚀 ~ SmartSignalList ~ smartSignals:", smartSignals);
