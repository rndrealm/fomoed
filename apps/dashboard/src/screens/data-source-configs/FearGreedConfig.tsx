...
export function FearGreedConfig({ setDataObject }: DataConfigComponentProps) {
    const [open, setOpen] = useState(false);
    const [selected, setSelected] = useState<string>("");

    useEffect(() => {
        const topicName = "cfgi_" + selected;

        if (selected) {
            setDataObject({ topic: [topicName, "cfgi"] });
        }
    ...
    return (
        <div className="space-y-4 p-4 max-w-sm">
            <div className="space-y-2">
                <Label htmlFor="cfgi-symbol" className="text-gray-400">
                    Symbol
                </Label>
                <Popover open={open} onOpenChange={setOpen}>
                    <PopoverTrigger asChild>
                        <Button
                            ...
                            id="cfgi-symbol"
                        >
                            {selected ? selected : "Select symbol"}
            ...
            {/* Add more Fear & Greed-specific configurations here */}
        </div>
    );
}
