...
const OpenOrdersTab = ({ userAddress, coinInfoMap }: OpenOrdersTabProps) => {
  const { data: orders } = useHyperliquidOpenOrders(userAddress, !!userAddress);
  ...
  if (!orders || orders.length === 0) {
    return (
      <div className="flex flex-col h-full">
        {/* Header */}
        <div
          className="grid grid-cols-[1.2fr_0.8fr_0.8fr_1fr_1fr_1.2fr_1fr_0.8fr_0.8fr] gap-4 sticky top-0 z-10 bg-[#121317] border-b border-[#1B1B1B] items-center"
          style={{
            height: "40px",
            paddingTop: "12px",
            paddingBottom: "12px",
        ...
        >
          ...
          <div className="text-[#84858C] text-[12px] font-medium text-left">Trigger Price</div>
          ...
          <div className="text-[#84858C] text-[12px] font-medium text-left">Time</div>
          <div className="text-[#84858C] text-[12px] font-medium text-left">Action</div>
        </div>
        ...
        {orders.map((order) => {
          ...
          return (
            <div
              ...
              className="grid grid-cols-[1.2fr_0.8fr_0.8fr_1fr_1fr_1.2fr_1fr_0.8fr_0.8fr] gap-4 hover:bg-[#1C1D21] transition-colors items-center"
              style={{
                height: "48px",
                paddingLeft: "12px",
                paddingRight: "12px",
            ...
            >
              ...
              {/* Trigger Price */}
              <span className="text-white text-[12px]">
                {order.triggerPx && order.triggerPx !== "0" ? formatPrice(order.triggerPx) : "-"}
              </span>
