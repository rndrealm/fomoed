...
import { WhaleTransaction, CoinDataInterface } from "@/services/queries/charts/types"; // Adjust import path
          ...
          <tbody>
            {transactions.map((item, index) => {
              ...
              const rowStyle = isHighValue
                ...
                : {
                    background: isLong 
                      ? 'linear-gradient(to right, #0C0C0C, #171717, #272B29, #48544F 60%, #7DC4A4)' 
                      : 'linear-gradient(to right, #0C0C0C, #171717, #3B221D, #983E2B 60%, #FF8970)'
                  };
                ...
                >
                  ...
                  <td className="border-y border-transparent py-2 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      {iconUrl ? (
                        <Image src={iconUrl} alt={`${item.token} logo`} width={24} height={24} className="rounded-full" />
                      ) : (
