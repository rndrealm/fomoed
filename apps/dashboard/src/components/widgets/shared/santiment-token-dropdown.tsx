...
export default function SanitmentTokenDropdown(props: IProps) {
  ...
  const activeCoin = options.find((coin) => coin.slug === value);
  ...
  const filteredCoins = useMemo(() => {
    ...
    return options.filter((coin) => {
      ...
      const symbolMatch = coin.slug.toLowerCase().includes(search);
      const tickerMatch = coin.ticker.toLowerCase().includes(search);
      return nameMatch || symbolMatch || tickerMatch;
    });
  }, [searchValue, options]);
