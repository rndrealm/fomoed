...
import { useGemachDepositToHyperliquid, useReadGemachBalance, useReadGemachUser } from "@/services/queries/gemach";
...
export function Hyperliquid(props: IProps) {
  ...
  const depositMutation = useGemachDepositToHyperliquid(session?.access_token);
  ...
  const onSubmit = (_values: InitialValues) => {
    const body = {
      ...
      address: address || "",
      gdexAddress: userData?.address || "",
    };

    depositMutation.mutate(body, {
      onSuccess: () => {
        toast.success("Deposit to Hyperliquid initiated. It may take 1-2 minutes to reflect in your account.");
        handleClose();
      },
      onError: () => {
        toast.error("Failed to initiate deposit to Hyperliquid. Please try again.");
      },
    });
  };
