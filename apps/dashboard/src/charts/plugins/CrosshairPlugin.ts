...
function renderXCrosshairLabel(
  chart: Chart,
  xCenterPixel: number,
  opts: LabelingOptions
) {
  ...
  const xVal = chart.scales[opts.scaleId].getValueForPixel(
    xCenterPixel
  ) as number;
...
function renderYCrosshairLabel(
  chart: Chart,
  yCenterPixel: number,
  opts: LabelingOptions
) {
...
function drawIntersections(
  ...
  for (const label of labelsToDraw) {
    ...
    if (chart._crosshairCache.opts.labelStackDirection === "horizontal") {
      ...
      ctx.fillText(
        label.value,
        chartArea.left + maxWidth - valWidth,
        chartArea.top + topAcc
      );
    }
...
export const CrosshairPlugin: Plugin = {
  ...
  afterInit: (chart: Chart_) => {
    const userOpts = (chart.options.plugins as any)[
      "crosshair"
    ] as CrosshairPluginConfig;
    ...
    function touchmoveHandler(e: TouchEvent) {
      const deltaX =
        e.touches[0].clientX - chart._crosshairCache.lastToucheventCoords.x;
      const deltaY =
        e.touches[0].clientY - chart._crosshairCache.lastToucheventCoords.y;
  ...
  beforeDestroy: (chart: Chart_) => {
    for (const { event, handler } of chart._crosshairCache
      .customEventHandlers) {
  ...
  afterDraw: (chart: Chart_) => {
    ...
    if (largerXVal && smallerXVal) {
      nearestXVal =
        Math.abs(largerXVal - xVal) < Math.abs(smallerXVal - xVal)
          ? largerXVal
          : smallerXVal;
    }
    ...
    const yScales = options.labels.filter(
      (i) => chart.scales[i.scaleId].axis === "y"
    );

    drawIntersections(chart, {
      xVal: nearestXVal,
      xPixel: nearestXPixel,
      labelOptions: yScales,
    });
  },
};
